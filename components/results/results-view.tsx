...
import { ArrowLeft, Zap, Search, Bot, Shield, Server } from "lucide-react"
    ...
    switch (level) {
      case "FAST":
        return {
          ...
          label: "FAST Path",
          description: "Simple query, cached results",
        }
      case "SMART":
        return {
          color: "bg-warning/10 text-warning border-warning/20",
          label: "SMART Path",
          description: "Moderate complexity, AI-enhanced",
        }
      case "DEEP":
        return {
          color: "bg-destructive/10 text-destructive border-destructive/20",
          label: "DEEP Path",
          description: "Complex analysis, full AI pipeline",
        }
      default:
        return { color: "bg-muted text-muted-foreground", label: level, description: "" }
    }
  }
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3 rounded-lg border border-border bg-card p-4">
        ...
        <div className="group relative">
          <span
            className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium cursor-help ${complexityBadge.color}`}
          >
            {complexityBadge.label}
          </span>
          {complexityBadge.description && (
        ...
        {/* Agents Used */}
        {results.metadata.agents_used && results.metadata.agents_used.length > 0 && (
          <>
            <div className="h-4 w-px bg-border hidden sm:block" />
            <div className="hidden sm:flex items-center gap-2">
              <Bot className="h-4 w-4 text-muted-foreground" />
              <div className="flex gap-1">
                {results.metadata.agents_used.map((agent) => (
      ...
      <div className="mb-6 flex items-start gap-3 rounded-lg border border-primary/20 bg-primary/5 p-4">
        ...
        <div className="text-sm">
          <p className="font-medium text-foreground">AI-Powered Results</p>
          <p className="text-muted-foreground mt-1">
            Recommendations are generated by our multi-agent AI system. Financial analysis is
            calculated based on your profile. Scores include Thompson Sampling for personalization
            and RAGAS for explanation quality.
          </p>
        </div>
      </div>
